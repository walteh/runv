// Code generated by mockery. DO NOT EDIT.

package mockgrpc

import (
	context "context"

	metadata "google.golang.org/grpc/metadata"

	mock "github.com/stretchr/testify/mock"
)

// MockBidiStreamingServer is an autogenerated mock type for the BidiStreamingServer type
type MockBidiStreamingServer[Req any, Res any] struct {
	mock.Mock
}

type MockBidiStreamingServer_Expecter[Req any, Res any] struct {
	mock *mock.Mock
}

func (_m *MockBidiStreamingServer[Req, Res]) EXPECT() *MockBidiStreamingServer_Expecter[Req, Res] {
	return &MockBidiStreamingServer_Expecter[Req, Res]{mock: &_m.Mock}
}

// Context provides a mock function with no fields
func (_m *MockBidiStreamingServer[Req, Res]) Context() context.Context {
	ret := _m.Called()

	if len(ret) == 0 {
		panic("no return value specified for Context")
	}

	var r0 context.Context
	if rf, ok := ret.Get(0).(func() context.Context); ok {
		r0 = rf()
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(context.Context)
		}
	}

	return r0
}

// MockBidiStreamingServer_Context_Call is a *mock.Call that shadows Run/Return methods with type explicit version for method 'Context'
type MockBidiStreamingServer_Context_Call[Req any, Res any] struct {
	*mock.Call
}

// Context is a helper method to define mock.On call
func (_e *MockBidiStreamingServer_Expecter[Req, Res]) Context() *MockBidiStreamingServer_Context_Call[Req, Res] {
	return &MockBidiStreamingServer_Context_Call[Req, Res]{Call: _e.mock.On("Context")}
}

func (_c *MockBidiStreamingServer_Context_Call[Req, Res]) Run(run func()) *MockBidiStreamingServer_Context_Call[Req, Res] {
	_c.Call.Run(func(args mock.Arguments) {
		run()
	})
	return _c
}

func (_c *MockBidiStreamingServer_Context_Call[Req, Res]) Return(_a0 context.Context) *MockBidiStreamingServer_Context_Call[Req, Res] {
	_c.Call.Return(_a0)
	return _c
}

func (_c *MockBidiStreamingServer_Context_Call[Req, Res]) RunAndReturn(run func() context.Context) *MockBidiStreamingServer_Context_Call[Req, Res] {
	_c.Call.Return(run)
	return _c
}

// Recv provides a mock function with no fields
func (_m *MockBidiStreamingServer[Req, Res]) Recv() (*Req, error) {
	ret := _m.Called()

	if len(ret) == 0 {
		panic("no return value specified for Recv")
	}

	var r0 *Req
	var r1 error
	if rf, ok := ret.Get(0).(func() (*Req, error)); ok {
		return rf()
	}
	if rf, ok := ret.Get(0).(func() *Req); ok {
		r0 = rf()
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(*Req)
		}
	}

	if rf, ok := ret.Get(1).(func() error); ok {
		r1 = rf()
	} else {
		r1 = ret.Error(1)
	}

	return r0, r1
}

// MockBidiStreamingServer_Recv_Call is a *mock.Call that shadows Run/Return methods with type explicit version for method 'Recv'
type MockBidiStreamingServer_Recv_Call[Req any, Res any] struct {
	*mock.Call
}

// Recv is a helper method to define mock.On call
func (_e *MockBidiStreamingServer_Expecter[Req, Res]) Recv() *MockBidiStreamingServer_Recv_Call[Req, Res] {
	return &MockBidiStreamingServer_Recv_Call[Req, Res]{Call: _e.mock.On("Recv")}
}

func (_c *MockBidiStreamingServer_Recv_Call[Req, Res]) Run(run func()) *MockBidiStreamingServer_Recv_Call[Req, Res] {
	_c.Call.Run(func(args mock.Arguments) {
		run()
	})
	return _c
}

func (_c *MockBidiStreamingServer_Recv_Call[Req, Res]) Return(_a0 *Req, _a1 error) *MockBidiStreamingServer_Recv_Call[Req, Res] {
	_c.Call.Return(_a0, _a1)
	return _c
}

func (_c *MockBidiStreamingServer_Recv_Call[Req, Res]) RunAndReturn(run func() (*Req, error)) *MockBidiStreamingServer_Recv_Call[Req, Res] {
	_c.Call.Return(run)
	return _c
}

// RecvMsg provides a mock function with given fields: m
func (_m *MockBidiStreamingServer[Req, Res]) RecvMsg(m any) error {
	ret := _m.Called(m)

	if len(ret) == 0 {
		panic("no return value specified for RecvMsg")
	}

	var r0 error
	if rf, ok := ret.Get(0).(func(any) error); ok {
		r0 = rf(m)
	} else {
		r0 = ret.Error(0)
	}

	return r0
}

// MockBidiStreamingServer_RecvMsg_Call is a *mock.Call that shadows Run/Return methods with type explicit version for method 'RecvMsg'
type MockBidiStreamingServer_RecvMsg_Call[Req any, Res any] struct {
	*mock.Call
}

// RecvMsg is a helper method to define mock.On call
//   - m any
func (_e *MockBidiStreamingServer_Expecter[Req, Res]) RecvMsg(m interface{}) *MockBidiStreamingServer_RecvMsg_Call[Req, Res] {
	return &MockBidiStreamingServer_RecvMsg_Call[Req, Res]{Call: _e.mock.On("RecvMsg", m)}
}

func (_c *MockBidiStreamingServer_RecvMsg_Call[Req, Res]) Run(run func(m any)) *MockBidiStreamingServer_RecvMsg_Call[Req, Res] {
	_c.Call.Run(func(args mock.Arguments) {
		run(args[0].(any))
	})
	return _c
}

func (_c *MockBidiStreamingServer_RecvMsg_Call[Req, Res]) Return(_a0 error) *MockBidiStreamingServer_RecvMsg_Call[Req, Res] {
	_c.Call.Return(_a0)
	return _c
}

func (_c *MockBidiStreamingServer_RecvMsg_Call[Req, Res]) RunAndReturn(run func(any) error) *MockBidiStreamingServer_RecvMsg_Call[Req, Res] {
	_c.Call.Return(run)
	return _c
}

// Send provides a mock function with given fields: _a0
func (_m *MockBidiStreamingServer[Req, Res]) Send(_a0 *Res) error {
	ret := _m.Called(_a0)

	if len(ret) == 0 {
		panic("no return value specified for Send")
	}

	var r0 error
	if rf, ok := ret.Get(0).(func(*Res) error); ok {
		r0 = rf(_a0)
	} else {
		r0 = ret.Error(0)
	}

	return r0
}

// MockBidiStreamingServer_Send_Call is a *mock.Call that shadows Run/Return methods with type explicit version for method 'Send'
type MockBidiStreamingServer_Send_Call[Req any, Res any] struct {
	*mock.Call
}

// Send is a helper method to define mock.On call
//   - _a0 *Res
func (_e *MockBidiStreamingServer_Expecter[Req, Res]) Send(_a0 interface{}) *MockBidiStreamingServer_Send_Call[Req, Res] {
	return &MockBidiStreamingServer_Send_Call[Req, Res]{Call: _e.mock.On("Send", _a0)}
}

func (_c *MockBidiStreamingServer_Send_Call[Req, Res]) Run(run func(_a0 *Res)) *MockBidiStreamingServer_Send_Call[Req, Res] {
	_c.Call.Run(func(args mock.Arguments) {
		run(args[0].(*Res))
	})
	return _c
}

func (_c *MockBidiStreamingServer_Send_Call[Req, Res]) Return(_a0 error) *MockBidiStreamingServer_Send_Call[Req, Res] {
	_c.Call.Return(_a0)
	return _c
}

func (_c *MockBidiStreamingServer_Send_Call[Req, Res]) RunAndReturn(run func(*Res) error) *MockBidiStreamingServer_Send_Call[Req, Res] {
	_c.Call.Return(run)
	return _c
}

// SendHeader provides a mock function with given fields: _a0
func (_m *MockBidiStreamingServer[Req, Res]) SendHeader(_a0 metadata.MD) error {
	ret := _m.Called(_a0)

	if len(ret) == 0 {
		panic("no return value specified for SendHeader")
	}

	var r0 error
	if rf, ok := ret.Get(0).(func(metadata.MD) error); ok {
		r0 = rf(_a0)
	} else {
		r0 = ret.Error(0)
	}

	return r0
}

// MockBidiStreamingServer_SendHeader_Call is a *mock.Call that shadows Run/Return methods with type explicit version for method 'SendHeader'
type MockBidiStreamingServer_SendHeader_Call[Req any, Res any] struct {
	*mock.Call
}

// SendHeader is a helper method to define mock.On call
//   - _a0 metadata.MD
func (_e *MockBidiStreamingServer_Expecter[Req, Res]) SendHeader(_a0 interface{}) *MockBidiStreamingServer_SendHeader_Call[Req, Res] {
	return &MockBidiStreamingServer_SendHeader_Call[Req, Res]{Call: _e.mock.On("SendHeader", _a0)}
}

func (_c *MockBidiStreamingServer_SendHeader_Call[Req, Res]) Run(run func(_a0 metadata.MD)) *MockBidiStreamingServer_SendHeader_Call[Req, Res] {
	_c.Call.Run(func(args mock.Arguments) {
		run(args[0].(metadata.MD))
	})
	return _c
}

func (_c *MockBidiStreamingServer_SendHeader_Call[Req, Res]) Return(_a0 error) *MockBidiStreamingServer_SendHeader_Call[Req, Res] {
	_c.Call.Return(_a0)
	return _c
}

func (_c *MockBidiStreamingServer_SendHeader_Call[Req, Res]) RunAndReturn(run func(metadata.MD) error) *MockBidiStreamingServer_SendHeader_Call[Req, Res] {
	_c.Call.Return(run)
	return _c
}

// SendMsg provides a mock function with given fields: m
func (_m *MockBidiStreamingServer[Req, Res]) SendMsg(m any) error {
	ret := _m.Called(m)

	if len(ret) == 0 {
		panic("no return value specified for SendMsg")
	}

	var r0 error
	if rf, ok := ret.Get(0).(func(any) error); ok {
		r0 = rf(m)
	} else {
		r0 = ret.Error(0)
	}

	return r0
}

// MockBidiStreamingServer_SendMsg_Call is a *mock.Call that shadows Run/Return methods with type explicit version for method 'SendMsg'
type MockBidiStreamingServer_SendMsg_Call[Req any, Res any] struct {
	*mock.Call
}

// SendMsg is a helper method to define mock.On call
//   - m any
func (_e *MockBidiStreamingServer_Expecter[Req, Res]) SendMsg(m interface{}) *MockBidiStreamingServer_SendMsg_Call[Req, Res] {
	return &MockBidiStreamingServer_SendMsg_Call[Req, Res]{Call: _e.mock.On("SendMsg", m)}
}

func (_c *MockBidiStreamingServer_SendMsg_Call[Req, Res]) Run(run func(m any)) *MockBidiStreamingServer_SendMsg_Call[Req, Res] {
	_c.Call.Run(func(args mock.Arguments) {
		run(args[0].(any))
	})
	return _c
}

func (_c *MockBidiStreamingServer_SendMsg_Call[Req, Res]) Return(_a0 error) *MockBidiStreamingServer_SendMsg_Call[Req, Res] {
	_c.Call.Return(_a0)
	return _c
}

func (_c *MockBidiStreamingServer_SendMsg_Call[Req, Res]) RunAndReturn(run func(any) error) *MockBidiStreamingServer_SendMsg_Call[Req, Res] {
	_c.Call.Return(run)
	return _c
}

// SetHeader provides a mock function with given fields: _a0
func (_m *MockBidiStreamingServer[Req, Res]) SetHeader(_a0 metadata.MD) error {
	ret := _m.Called(_a0)

	if len(ret) == 0 {
		panic("no return value specified for SetHeader")
	}

	var r0 error
	if rf, ok := ret.Get(0).(func(metadata.MD) error); ok {
		r0 = rf(_a0)
	} else {
		r0 = ret.Error(0)
	}

	return r0
}

// MockBidiStreamingServer_SetHeader_Call is a *mock.Call that shadows Run/Return methods with type explicit version for method 'SetHeader'
type MockBidiStreamingServer_SetHeader_Call[Req any, Res any] struct {
	*mock.Call
}

// SetHeader is a helper method to define mock.On call
//   - _a0 metadata.MD
func (_e *MockBidiStreamingServer_Expecter[Req, Res]) SetHeader(_a0 interface{}) *MockBidiStreamingServer_SetHeader_Call[Req, Res] {
	return &MockBidiStreamingServer_SetHeader_Call[Req, Res]{Call: _e.mock.On("SetHeader", _a0)}
}

func (_c *MockBidiStreamingServer_SetHeader_Call[Req, Res]) Run(run func(_a0 metadata.MD)) *MockBidiStreamingServer_SetHeader_Call[Req, Res] {
	_c.Call.Run(func(args mock.Arguments) {
		run(args[0].(metadata.MD))
	})
	return _c
}

func (_c *MockBidiStreamingServer_SetHeader_Call[Req, Res]) Return(_a0 error) *MockBidiStreamingServer_SetHeader_Call[Req, Res] {
	_c.Call.Return(_a0)
	return _c
}

func (_c *MockBidiStreamingServer_SetHeader_Call[Req, Res]) RunAndReturn(run func(metadata.MD) error) *MockBidiStreamingServer_SetHeader_Call[Req, Res] {
	_c.Call.Return(run)
	return _c
}

// SetTrailer provides a mock function with given fields: _a0
func (_m *MockBidiStreamingServer[Req, Res]) SetTrailer(_a0 metadata.MD) {
	_m.Called(_a0)
}

// MockBidiStreamingServer_SetTrailer_Call is a *mock.Call that shadows Run/Return methods with type explicit version for method 'SetTrailer'
type MockBidiStreamingServer_SetTrailer_Call[Req any, Res any] struct {
	*mock.Call
}

// SetTrailer is a helper method to define mock.On call
//   - _a0 metadata.MD
func (_e *MockBidiStreamingServer_Expecter[Req, Res]) SetTrailer(_a0 interface{}) *MockBidiStreamingServer_SetTrailer_Call[Req, Res] {
	return &MockBidiStreamingServer_SetTrailer_Call[Req, Res]{Call: _e.mock.On("SetTrailer", _a0)}
}

func (_c *MockBidiStreamingServer_SetTrailer_Call[Req, Res]) Run(run func(_a0 metadata.MD)) *MockBidiStreamingServer_SetTrailer_Call[Req, Res] {
	_c.Call.Run(func(args mock.Arguments) {
		run(args[0].(metadata.MD))
	})
	return _c
}

func (_c *MockBidiStreamingServer_SetTrailer_Call[Req, Res]) Return() *MockBidiStreamingServer_SetTrailer_Call[Req, Res] {
	_c.Call.Return()
	return _c
}

func (_c *MockBidiStreamingServer_SetTrailer_Call[Req, Res]) RunAndReturn(run func(metadata.MD)) *MockBidiStreamingServer_SetTrailer_Call[Req, Res] {
	_c.Run(run)
	return _c
}

// NewMockBidiStreamingServer creates a new instance of MockBidiStreamingServer. It also registers a testing interface on the mock and a cleanup function to assert the mocks expectations.
// The first argument is typically a *testing.T value.
func NewMockBidiStreamingServer[Req any, Res any](t interface {
	mock.TestingT
	Cleanup(func())
}) *MockBidiStreamingServer[Req, Res] {
	mock := &MockBidiStreamingServer[Req, Res]{}
	mock.Mock.Test(t)

	t.Cleanup(func() { mock.AssertExpectations(t) })

	return mock
}
