...
func ConvertIOOut(ctx context.Context, rvio runtime.IO) (*runvv1.RuncIO, error) {
	res := &runvv1.RuncIO{}
...
func NewServerIOFromClient(ctx context.Context, rvio *runvv1.RuncIO) (runtime.IO, error) {
	...
	var forwarder runtime.ListenForwarders
	...
	case runvv1.RuncIO_Vsock_case:
		...
		forwarder, err = runtime.NewVsockForwarder(ctx, ctxId, stdinPort, stdoutPort, stderrPort, dialFunc, listenFunc)
	case runvv1.RuncIO_Unix_case:
		stdinPath := rvio.GetUnix().GetStdinPath()
		stdoutPath := rvio.GetUnix().GetStdoutPath()
		stderrPath := rvio.GetUnix().GetStderrPath()
		forwarder, err = runtime.NewUnixForwarder(ctx, stdinPath, stdoutPath, stderrPath)
	...
	guestIO, err := runtime.NewGuestIO(ctx, forwarder, opts...)
	if err != nil {
		return nil, err
	}
