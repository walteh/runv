...
func NewRunmVMRuntime[VM vmm.VirtualMachine](ctx context.Context, hpv vmm.Hypervisor[VM], publisher shim.Publisher, spec *specs.Spec) (*RunmVMRuntime[VM], error) {

	runGroup := run.New()

	vm, err := vmm.NewContainerizedVirtualMachine(ctx, hpv, vmm.ContainerizedVMConfig{
		Spec: spec,
	})
	if err != nil {
		return nil, err
